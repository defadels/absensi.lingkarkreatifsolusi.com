<x-layouts.app>
    ...
    <div class="space-y-4 sm:space-y-6">
        <!-- Greeting Card -->
        <div class="relative overflow-hidden rounded-2xl p-4 sm:p-6" style="background: linear-gradient(135deg, #1e1b4b 0%, #312e81 50%, #1e1b4b 100%); border: 1px solid rgba(99,102,241,0.3);">
            ...
            <div class="relative z-10 flex items-start justify-between gap-3">
                <div class="min-w-0">
                    <p class="text-indigo-300 text-xs sm:text-sm">Selamat datang,</p>
                    <h2 class="text-xl sm:text-2xl font-bold text-white mt-1 truncate">{{ auth()->user()->name }} 👋</h2>
                    @if($pegawai && $pegawai->jabatan)
                        <p class="text-indigo-300 text-xs sm:text-sm mt-1 truncate">{{ $pegawai->jabatan }} • {{ $pegawai->divisi }}</p>
                    @else
                        <p class="text-amber-400 text-xs sm:text-sm mt-1">⚠ Data jabatan belum dilengkapi</p>
                    @endif
                </div>
                <div class="text-right flex-shrink-0">
                    <p class="text-2xl sm:text-3xl font-bold text-white tabular-nums" id="dashTime">{{ $jamSekarang }}</p>
                    <p class="text-indigo-300 text-xs hidden sm:block">{{ now()->isoFormat('dddd, D MMMM Y') }}</p>
                    <p class="text-indigo-300 text-xs sm:hidden">{{ now()->isoFormat('D MMM Y') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
            ...
            <div class="glass-card rounded-2xl p-4 sm:p-5 stat-card">
                ...
                @else
                    <p class="text-xl sm:text-2xl font-bold text-indigo-400">Belum absen</p>
            ...
            <div class="glass-card rounded-2xl p-4 sm:p-5 stat-card">
                ...
                @elseif($sudahMasuk)
                    <p class="text-xl sm:text-2xl font-bold text-indigo-400">Belum pulang</p>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
            <a href="{{ route('pegawai.absensi.masuk') }}" class="glass-card rounded-xl p-3 sm:p-4 text-center hover:bg-indigo-600/20 transition-all duration-200 group">
            ...
            <a href="{{ route('pegawai.absensi.pulang') }}" class="glass-card rounded-xl p-3 sm:p-4 text-center hover:bg-violet-600/20 transition-all duration-200 group">
            ...
            <a href="{{ route('pegawai.izin.create') }}" class="glass-card rounded-xl p-3 sm:p-4 text-center hover:bg-teal-600/20 transition-all duration-200 group">
            ...
            <a href="{{ route('pegawai.riwayat') }}" class="glass-card rounded-xl p-3 sm:p-4 text-center hover:bg-amber-600/20 transition-all duration-200 group">
