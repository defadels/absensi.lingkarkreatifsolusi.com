<x-layouts.app>
    ...
    <div class="max-w-4xl mx-auto space-y-5">
        <a href="{{ route('admin.absensi.index') }}" class="text-indigo-400 hover:text-white transition-colors text-sm">← Kembali ke Daftar</a>
        ...
        <div class="glass-card rounded-2xl p-5">
            <div class="flex items-center gap-4">
                ...
                <div class="text-right">
                    ...
                    <p class="text-xs text-indigo-400 mt-2">{{ \Carbon\Carbon::parse($absensi->tanggal)->isoFormat('dddd, D MMMM Y') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="glass-card rounded-xl p-4">
                ...
                <p class="text-xl font-bold text-white">{{ $absensi->jam_masuk ? \Carbon\Carbon::parse($absensi->jam_masuk)->format('H:i') : '—' }}</p>
            </div>
            <div class="glass-card rounded-xl p-4">
                <p class="text-xs text-indigo-400 mb-1">Jam Pulang</p>
                <p class="text-xl font-bold text-white">{{ $absensi->jam_pulang ? \Carbon\Carbon::parse($absensi->jam_pulang)->format('H:i') : '—' }}</p>
            </div>
            <div class="glass-card rounded-xl p-4">
                <p class="text-xs text-indigo-400 mb-1">Jarak Masuk</p>
                <p class="text-xl font-bold {{ $absensi->jarak_masuk_meter <= $absensi->lokasi_kerja->radius_meter ? 'text-emerald-400' : 'text-red-400' }}">
                    {{ $absensi->jarak_masuk_meter ? $absensi->jarak_masuk_meter . 'm' : '—' }}
                </p>
            </div>
            <div class="glass-card rounded-xl p-4">
                <p class="text-xs text-indigo-400 mb-1">Jarak Pulang</p>
                <p class="text-xl font-bold {{ $absensi->jarak_pulang_meter && $absensi->jarak_pulang_meter <= $absensi->lokasi_kerja->radius_meter ? 'text-emerald-400' : 'text-indigo-400' }}">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div class="glass-card rounded-2xl p-5">
                ...
                <div class="rounded-xl overflow-hidden border border-indigo-700/30 mb-3">
                    <div id="verifyMap" class="h-52"></div>
                </div>
